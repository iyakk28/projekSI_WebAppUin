            ...
            <div class="lg:col-span-8 space-y-8">
              ...
              <div class="space-y-6 pt-6 border-t border-slate-100">
                ...
                <div v-else class="space-y-4">
                  ...
                  <div
                    v-if="logStore.hasMore"
                    class="flex justify-center mt-4"
                  >
                    <button
                      @click="loadMoreLogs"
                      :disabled="logStore.loading"
                      class="flex items-center gap-2 px-6 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-[#3b5988] transition-all disabled:opacity-50"
                    >
                      <Icon
                        v-if="logStore.loading"
                        name="heroicons:arrow-path"
...
<script setup lang="ts">
  ...
  const loadMoreLogs = async () => {
    if (selectedDoc.value?.id) {
      await logStore.fetchLogs(selectedDoc.value.id, true);
    }
  };
