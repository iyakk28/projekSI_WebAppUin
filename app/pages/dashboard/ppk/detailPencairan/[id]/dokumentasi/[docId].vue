    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 py-8" v-if="doc">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div v-if="isProcessed" class="p-4 rounded-2xl border" :class="processedBannerClass">
          ...
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="p-8">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div v-for="img in visualFiles" :key="img.field" class="group relative aspect-video bg-slate-100 rounded-xl overflow-hidden border border-slate-200 shadow-sm transition hover:border-[#3b5988]/30">
                  <template v-if="blobUrls[img.field]">
                    <img :src="blobUrls[img.field]" class="w-full h-full object-contain" />
                  </template>
                  <div v-else class="flex items-center justify-center h-full">
                     <div class="w-6 h-6 border-2 border-slate-200 border-t-[#3b5988] rounded-full animate-spin"></div>
                  </div>

                  <div v-if="blobUrls[img.field]" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                    <a :href="blobUrls[img.field]" target="_blank" class="px-4 py-2 bg-white rounded-lg text-xs font-bold text-slate-800 shadow-xl hover:scale-105 transition">Buka Berkas</a>
                  </div>
                  <div class="absolute bottom-2 left-2 px-2 py-1 bg-black/60 text-white text-[10px] rounded uppercase font-bold backdrop-blur-sm">{{ img.label }}</div>
                </div>
              </div>
              ...
              <div class="mt-12 space-y-12">
                ...
                <div v-if="type === 'tagihan'">
                  ...
                  <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                      <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Nama Penerima</p>
                        <p class="font-bold text-slate-700 text-sm">{{ doc.namaPenerima }}</p>
                      </div>
                      <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Bank / Rekening</p>
                        <p class="font-bold text-slate-700 text-sm">{{ doc.bankPenerima }} - {{ doc.rekeningPenerima }}</p>
                      </div>
                      <div class="sm:col-span-2 pt-4 border-t border-slate-200/50">
                        <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Nominal Tagihan</p>
                        <p class="font-black text-2xl text-[#d1a82a]">{{ formatRp(doc.nominal) }}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-if="doc.tokoNama || doc.tokoAlamat" class="mb-8">
                    <h3 class="text-sm font-bold text-slate-800 uppercase mb-4 flex items-center gap-2">
                      <Icon name="heroicons:shopping-bag" class="w-4 h-4 text-slate-400" />
                      Rincian Toko / Penyedia
                    </h3>
                    <div class="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                      <div v-if="doc.tokoNama">
                        <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Nama Toko</p>
                        <p class="font-semibold text-slate-700 text-sm">{{ doc.tokoNama }}</p>
                      </div>
                      <div v-if="doc.tokoAlamat">
                        <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Alamat Toko</p>
                        <p class="font-medium text-slate-600 text-sm leading-relaxed">{{ doc.tokoAlamat }}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <!-- Foto Deskripsi -->
                <div v-if="type === 'foto'">
                  <h3 class="text-sm font-bold text-slate-800 uppercase mb-4">Deskripsi Dokumentasi</h3>
                ...
                <div v-if="type === 'tagihan'">
                   <h3 class="text-sm font-bold text-slate-800 uppercase mb-4 flex items-center gap-2">
                      <Icon name="heroicons:document-duplicate" class="w-4 h-4 text-slate-400" />
                      Dokumen Pendukung
                   </h3>
                   <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                     <div v-for="file in supportingFiles" :key="file.label" class="p-4 bg-white rounded-xl border border-slate-200 flex flex-col justify-between hover:shadow-md transition">
                       <div>
                         <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">{{ file.label }}</p>
                         <p class="text-[10px] text-slate-600 font-bold truncate mb-3" :class="file.value ? 'text-slate-800' : 'text-slate-400 italic'">
                           {{ file.value || 'Tidak ada data' }}
                         </p>
                       </div>
                       <template v-if="blobUrls[file.field]">
                         <a :href="blobUrls[file.field]" target="_blank" class="text-[10px] font-bold text-[#3b5988] flex items-center gap-1.5 px-3 py-1.5 bg-[#3b5988]/5 rounded-lg hover:bg-[#3b5988]/10 transition">
                           <Icon name="heroicons:arrow-top-right-on-square" class="w-3.5 h-3.5" />
                           Lihat Berkas
                         </a>
                       </template>
                       <p v-else-if="file.originalUrl" class="text-[10px] text-slate-400 flex items-center gap-2">
                         <div class="w-3 h-3 border-2 border-slate-200 border-t-[#3b5988] rounded-full animate-spin"></div>
                         Loading...
                       </p>
                       <p v-else class="text-[10px] text-slate-400 italic">Berkas tidak diunggah</p>
                     </div>
                   </div>
                </div>

                <!-- Info Pembayaran PPK (If Paid) -->
                <div v-if="type === 'tagihan' && doc.infoPembayaran" class="pt-8 border-t border-slate-200">
                    <h3 class="text-sm font-bold text-emerald-700 uppercase mb-4 flex items-center gap-2">
                        <Icon name="heroicons:credit-card" class="w-5 h-5" />
                        Informasi Realisasi Pembayaran
                    </h3>
                    <div class="p-6 bg-emerald-50 rounded-2xl border border-emerald-100 flex flex-col sm:flex-row gap-8">
                        <div class="flex-1 space-y-4">
                            <div>
                                <p class="text-[10px] font-bold text-emerald-600 uppercase mb-1">Tanggal Bayar</p>
                                <p class="font-bold text-slate-800 text-sm">{{ formatDate(doc.infoPembayaran.tanggal) }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-bold text-emerald-600 uppercase mb-1">Catatan PPK</p>
                                <p class="font-medium text-slate-700 text-sm italic">"{{ doc.infoPembayaran.catatan }}"</p>
                            </div>
                        </div>
                        <div v-if="blobUrls['paymentProof']" class="w-full sm:w-48 aspect-video sm:aspect-square bg-white rounded-xl border border-emerald-200 overflow-hidden relative group">
                            <img :src="blobUrls['paymentProof']" class="w-full h-full object-cover" />
                            <a :href="blobUrls['paymentProof']" target="_blank" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-[10px] font-bold text-white uppercase">Buka Bukti Bayar</a>
                        </div>
                    </div>
                </div>
              </div>
        ...
        <div class="space-y-6">
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sticky top-24">
            ...
            <div v-if="!isProcessed" class="space-y-4">
              ...
              <!-- Action Form -->
              <div v-if="selectedAction" class="mt-8 pt-6 border-t border-slate-100 space-y-6">
                <!-- Confirmation State (Custom UI No Alert) -->
                <div v-if="showConfirmation" class="p-4 bg-slate-900 rounded-xl text-white space-y-4 animate-in fade-in slide-in-from-top-2">
                    <div class="flex gap-3">
                        <Icon name="heroicons:question-mark-circle" class="w-5 h-5 text-amber-400 flex-shrink-0" />
                        <div>
                            <p class="text-sm font-bold">Konfirmasi Aksi?</p>
                            <p class="text-[10px] opacity-70">Apakah Anda yakin data ini sudah benar dan ingin memprosesnya?</p>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <button @click="showConfirmation = false" class="flex-1 py-2 rounded-lg bg-white/10 text-xs font-bold hover:bg-white/20">Batal</button>
                        <button @click="submitFinalAction" class="flex-1 py-2 rounded-lg bg-emerald-500 text-xs font-bold hover:bg-emerald-600">Ya, Proses</button>
                    </div>
                </div>
            ...
            <div v-else class="p-6 text-center space-y-4">
                <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto border border-slate-100">
                    <Icon :name="isSuccessAction ? 'heroicons:check-badge' : 'heroicons:lock-closed'" class="w-8 h-8" :class="isSuccessAction ? 'text-emerald-500' : 'text-slate-300'" />
                </div>
                <div>
                    <p class="font-bold text-slate-800 text-sm">{{ isSuccessAction ? 'Berkas Berhasil Diproses' : 'Berkas Dikembalikan' }}</p>
                    <p class="text-xs text-slate-500 mt-1">Aksi untuk berkas ini sudah dikunci karena statusnya sudah final di tahap Anda.</p>
                </div>
            </div>
          </div>

          <!-- Feedback Messages -->
          <div v-if="successMsg" class="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex gap-3 text-emerald-800 animate-in fade-in">
            <Icon name="heroicons:check-circle" class="w-5 h-5 flex-shrink-0" />
            <p class="text-sm font-medium">{{ successMsg }}</p>
          </div>
          <div v-if="error" class="bg-red-50 border border-red-200 rounded-2xl p-4 flex gap-3 text-red-800 animate-in shake">
...
const loadAllBlobs = async () => {
  ...
  const fieldsToLoad = type === 'foto' 
    ? ['fileUrl'] 
    : ['strukFileUrl', 'fotoBarangUrl', 'skFileUrl', 'npwpFileUrl', 'ktpFileUrl', 'spmtFileUrl', 'amprahFileUrl', 'bukuRekeningFileUrl'];
...
const visualFiles = computed(() => {
  ...
  const files = [
    ...
    { label: doc.value.tipeTagihan === 'Jasa' ? 'Foto Dokumentasi Jasa' : 'Foto Barang', field: 'fotoBarangUrl', originalUrl: doc.value.fotoBarangUrl },
  ];

  if (doc.value.infoPembayaran) {
      files.push({ label: 'Bukti Pembayaran PPK', field: 'paymentProof', originalUrl: doc.value.infoPembayaran.buktiUrl });
  }
...
const supportingFiles = computed(() => {
  ...
  return [
    { label: 'SK / Penugasan', value: doc.value.skNomor, field: 'skFileUrl', originalUrl: doc.value.skFileUrl },
    { label: 'NPWP Penerima', value: doc.value.npwpNomor, field: 'npwpFileUrl', originalUrl: doc.value.npwpFileUrl },
    { label: 'KTP Penerima', value: doc.value.ktpNomor, field: 'ktpFileUrl', originalUrl: doc.value.ktpFileUrl },
    ...
    { label: 'Buku Rekening', value: 'Terlampir', field: 'bukuRekeningFileUrl', originalUrl: doc.value.bukuRekeningFileUrl }
  ];
});
...
const isProcessed = computed(() => {
    ...
    if (type === 'foto') return doc.value.status === 'DITERIMA' || doc.value.status === 'DITOLAK' || doc.value.status === 'REVISI';
    return doc.value.statusTagihan === 'SELESAI' || doc.value.statusTagihan === 'DITOLAK' || doc.value.statusTagihan === 'DIKEMBALIKAN';
});

const isSuccessAction = computed(() => {
    if (!doc.value) return false;
    return doc.value.status === 'DITERIMA' || doc.value.statusTagihan === 'SELESAI' || doc.value.statusTagihan === 'TERVERIFIKASI';
});

// Banner Metadata
const processedBannerClass = computed(() => {
    if (isSuccessAction.value) return 'bg-emerald-50 border-emerald-100 text-emerald-800';
    return 'bg-orange-50 border-orange-100 text-orange-800';
});

const processedIcon = computed(() => {
    return isSuccessAction.value ? 'heroicons:check-circle' : 'heroicons:exclamation-triangle';
});

const processedTitle = computed(() => {
    if (type === 'tagihan' && doc.value?.statusTagihan === 'SELESAI') return 'Tagihan Telah Dibayarkan';
...
const processedSubtitle = computed(() => {
    if (type === 'tagihan' && doc.value?.statusTagihan === 'SELESAI') return 'Status pembayaran sudah final dan tercatat di sistem.';
    if (isSuccessAction.value) return 'Berkas ini telah divalidasi dan tidak memerlukan aksi lebih lanjut.';
