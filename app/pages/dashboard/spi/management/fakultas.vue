    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <h1 class="text-2xl font-bold text-slate-900">Manajemen Fakultas</h1>
          <p class="text-slate-500 text-sm">Tambah dan kelola data fakultas universitas</p>
        </div>
      </div>

      <!-- Form Tambah -->
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h2 class="text-lg font-bold text-slate-900 mb-4">Tambah Fakultas Baru</h2>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
              <label class="text-sm font-medium text-slate-700">Nama Fakultas</label>
              <input
                v-model="form.nama"
                @input="errorMessage = ''"
                type="text"
            ...
            <div class="space-y-1">
              <label class="text-sm font-medium text-slate-700">Kode Fakultas</label>
              <input
                v-model="form.kode"
                @input="errorMessage = ''"
                type="text"
                ...
                class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-[#c41e3a]/20 focus:border-[#c41e3a] outline-none transition-all"
                :class="{ 'border-red-300 bg-red-50/30': errorMessage && (errorMessage.includes('Kode') || errorMessage.includes('FT')) }"
                required
              />
            </div>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-100">
          <h2 class="text-lg font-bold text-slate-900">Daftar Fakultas</h2>
        </div>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-slate-50 text-slate-600 text-sm font-semibold">
                ...
                <th class="px-6 py-4 border-b">Nama Fakultas</th>
                <th class="px-6 py-4 border-b">Kode</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              <tr v-for="(item, index) in store.fakultas" :key="item.id" class="hover:bg-slate-50 transition-colors">
              ...
              <tr v-if="store.fakultas.length === 0">
                <td colspan="3" class="px-6 py-8 text-center text-slate-500 italic">Belum ada data fakultas</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const store = useSpiFakultasStore();
...
const handleSubmit = async () => {
  errorMessage.value = '';
  const res = await store.addFakultas({ ...form.value });
  if (res.success) {
    form.value = { nama: '', kode: '' };
    alert('Fakultas berhasil ditambahkan!');
  } else {
    errorMessage.value = res.message || 'Gagal menambahkan fakultas';
  }
};
</script>
