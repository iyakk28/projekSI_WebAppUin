...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!pengajuanId || !requestedPath) {
      console.error("[API PPK File] ❌ Parameter tidak lengkap", {
        pengajuanId,
        requestedPath,
      });
      throw createError({
        statusCode: 400,
        statusMessage: "ID Pengajuan dan Path File wajib diisi.",
      });
    }
    ...
    const normalize = (p: string | null | undefined) => {
      ...
      return path
        .resolve(process.cwd(), cleanP.replace(/\\/g, "/"))
        .toLowerCase();
    };

    const targetPathNormalized = normalize(requestedPath);
    console.log(
      "[API PPK File] Requested Path (Normalized):",
    ...
    const isOwner = await db.transaction(async (tx) => {
      ...
      if (!rab) {
        console.error(
          "[API PPK File] ❌ Pengajuan tidak ditemukan atau beda fakultas",
          { pengajuanId, userFakultasId: user.fakultasId },
        );
        return false;
      }
      ...
      if (kegiatan) {
        ...
        const docs = await tx
          .select({ fileUrl: dokumentasiKegiatanTable.fileUrl })
        ...
        if (tIds.length > 0) {
          const payments = await tx
            .select({ bukti: pembayaranTable.buktiTransferUrl })
            .from(pembayaranTable)
            .where(inArray(pembayaranTable.tagihanId, tIds));
    ...
    if (!isOwner) {
      console.error(
        "[API PPK File] ❌ Akses ditolak: Path tidak terdaftar untuk pengajuan ini",
      );
      throw createError({
        statusCode: 403,
        statusMessage: "Anda tidak memiliki izin untuk mengakses file ini.",
      });
    }
